...
if [[ "$MODE" = "webporthttp" ]]; then
  ...
  if [[ -z "$port_http" ]]; then
    ...
    if [[ "$JEXBOSS" == "1" ]]; then
      ...
      echo -e "${OKGREEN}====================================================================================${RESET}•x${OKGREEN}[`date +"%Y-%m-%d](%H:%M)"`${RESET}x•"
      cd /tmp/
      python3 $INSTALL_DIR/plugins/jexboss/jexboss.py -u http://$TARGET:${PORT} | tee $LOOT_DIR/web/jexboss-$TARGET-port${PORT}.raw
    ...
    if [[ "$SMUGGLER" = "1" ]]; then
      ...
      echo -e "${OKGREEN}====================================================================================${RESET}•x${OKGREEN}[`date +"%Y-%m-%d](%H:%M)"`${RESET}x•"
      python3 $INSTALL_DIR/plugins/smuggler/smuggler.py --no-color -u http://$TARGET:${PORT} | tee $LOOT_DIR/web/smuggler-$TARGET-port${PORT}.txt
    fi
    if [[ "$NUCLEI" = "1" ]]; then
      echo -e "${OKGREEN}====================================================================================${RESET}•x${OKGREEN}[`date +"%Y-%m-%d](%H:%M)"`${RESET}x•"
  ...
  echo "[i] Scanning: http://$TARGET:$PORT/"
    sudo python3 $INSTALL_DIR/bin/zap-scan.py "http://$TARGET:$PORT/" 
    DATE=$(date +"%Y%m%d%H%M")
